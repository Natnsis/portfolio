...
const Hero = () => {
  ...
  return (
    <div
      id="home"
      className="flex items-center justify-center mt-10 md:mt-20 px-4"
    >
      <div className="flex flex-col items-center gap-6 md:gap-8 max-w-full">
            ...
            <div key={index}>
              <HoverCard openDelay={0.2} closeDelay={100}>
                <HoverCardTrigger>
                  <h.icon
                    size={24}
                    className="md:w-8 md:h-8 w-6 h-6 hover:opacity-60 transition-opacity"
                  />
                </HoverCardTrigger>
                <HoverCardContent className="text-sm max-w-62.5">
        ...
        <h1 className="text-5xl md:text-7xl lg:text-9xl text-center leading-tight">
          Natnael Sisay
        </h1>
        <p className="w-full md:w-[75%] text-center text-sm md:text-base leading-relaxed px-2">
          ...
          <Badge
            variant="outline"
            className="text-sm md:text-lg rounded-full border-black mx-1 pt-1"
          >
            DEVELOPER
          </Badge>{" "}
          WHO WORKS WITH A VARIETY OF CONSUMER FACING BRANDS FROM AROUND THE
          WORLD
        </p>
      </div>
    </div>
